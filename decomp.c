...
//    SafeRead(h, pr_globals, numpr_globals * 2);
...
#define MAX_NO_LOCAL_IMMEDIATES 65536 /* dln: increased, not enough! */
...
void DecompileDecompileStatement(dfunction_t * df, dstatement_t * s, int *indent)
{
		...
		(OP2_AND <= s->op && s->op <= OP2_BITOR))
	{
		...
		{
			DecompileIndent(*indent);
			fprintf(Decompileofile, "%s = %s %s %s;\n", arg3, arg1, pr_opcodes[s->op].name, arg2); // Opname to Name
		}
		else
		{
			sprintf(line, "(%s %s %s)", arg1, pr_opcodes[s->op].name, arg2); // Opname to Name
...
void DecompilePrintStatement(dstatement_t * s)
{
    ...
    printf("%4i : %s ", (int)(s - statements), pr_opcodes[s->op].name);
